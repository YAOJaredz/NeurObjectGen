...
def main(stim_limit, flux_lora_path=FLUX_LORA_DEFAULT_PATH):
    ...
    # ── FLUX pipeline ─────────────────────────────────────────────────────────
    pipe, image_proj = load_pipeline(device=device, default_scale=1.0, flux_lora_path=flux_lora_path)
    if flux_lora_path is not None:
        print(f"[generate_hvm_obj] using FLUX backbone LoRA: {flux_lora_path}")
