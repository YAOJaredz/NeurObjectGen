"""Train a MultiHeadTransformer: shared backbone → obj-SigLIP and CLIP heads.

Identical to train_multihead.py except the SigLIP target is the bbox-crop
embedding (HVM_OBJ_SIGLIP_EMBEDDINGS_PATH) rather than the full-image one.

Usage:
    python train/train_multihead_obj.py --dataset hvm --use-category
"""
...
from config_const import (
    ...
    HVM_OBJ_SIGLIP_EMBEDDINGS_PATH, HVM_CLIP_EMBEDS_PATH,
)
...
class HVMObjMultiHeadDataset(Dataset):
    """Yields (neural, {'siglip': obj_siglip, 'clip_short': clip}, cat_idx)."""

    def __init__(self, neural, obj_siglip, clip, cat):
        self.neural = neural
        self.cat    = cat
        self.targets = {"siglip": obj_siglip, "clip_short": clip}
...
def make_obj_loaders(batch_size: int = 64, seed: int = SEED):
    ...
    clip_t       = F.normalize(torch.load(HVM_CLIP_EMBEDS_PATH, weights_only=True), dim=-1)  # (450, 768)

    assert obj_siglip_t.shape == (HVM_N_STIMULI, 1152), obj_siglip_t.shape
    assert clip_t.shape       == (HVM_N_STIMULI, 768),  clip_t.shape
    ...
    def make(idx, shuffle):
        ds = HVMObjMultiHeadDataset(
            neural_t[idx], obj_siglip_t[idx], clip_t[idx], cat_indices[idx]
        )
...
def run_dir(args):
    tag = "hvm_obj_cat" if args.use_category else "hvm_obj"
...
def train(args):
    ...
    print(f"MultiHeadTransformer (obj-SigLIP) | params: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            pred = model(x, cat if args.use_category else None)
        ...
        ps  = torch.cat(preds_sig);  gs = torch.cat(gt_sig)
        pc  = torch.cat(preds_clip); gc = torch.cat(gt_clip)

        afc_sig  = two_afc_identification(ps, gs)
        afc_clip = two_afc_identification(pc, gc)
        cos_sig  = F.cosine_similarity(ps, gs, dim=-1).mean().item()
        cos_clip = F.cosine_similarity(pc, gc, dim=-1).mean().item()
        mean_cos = (cos_sig + cos_clip) / 2.0

        print(
            f"epoch {epoch:3d}/{args.epochs}  "
            f"loss={train_loss:.4f} (sig={train_l_sig:.3f} clip={train_l_clip:.3f})  "
            f"val: obj_sig_cos={cos_sig:.3f} clip_cos={cos_clip:.3f} "
            f"obj_sig_2afc={afc_sig:.3f} clip_2afc={afc_clip:.3f}  crit(cos)={mean_cos:.4f}"
        )
    ...
    print(
        f"\nTest (N={len(ts)}):\n"
        f"  Obj-SigLIP: cos={test_cos_sig:.4f}  2AFC={test_afc_sig:.3f}"
